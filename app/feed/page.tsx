...
import { ZCreateFeed, TCreateFeed, ZCreateComment, TCreateComment } from "@/validators/feed";
import {
  useCreateFeed,
  useGetFeeds,
  useCreateComment,
...
function getImageUrl(path?: string | null) {
  ...
  const base = process.env.NEXT_PUBLIC_BASE_API_URL || "http://localhost:8000/api/v1";
...
function FeedCard({
  ...
  const { mutate: toggleLike, isPending: liking } = useToggleFeedLike();
  const { mutate: deleteFeed } = useDeleteFeed();
  const { mutate: createComment, isPending: commenting } = useCreateComment();
  const { data: commentsData } = useGetCommentsByFeed(showComments ? feed.id : "");
  const comments = commentsData?.data || [];

  const commentForm = useForm<TCreateComment>({
    resolver: zodResolver(ZCreateComment),
    defaultValues: { feedId: feed.id, content: "", parentId: null },
  });

  const onSubmitComment = (data: TCreateComment) => {
    createComment(data, {
      onSuccess: () => {
        commentForm.reset({ feedId: feed.id, content: "", parentId: null });
      },
    });
  };

  const liked = feed.likes?.some((l: any) => l.userId === currentUserId);
  const likeCount = feed._count?.likes ?? 0;
  const commentCount = feed._count?.comments ?? 0;
  const isOwner = feed.authorId === currentUserId;
  ...
  return (
    <div className="_feed_inner_timeline_post_area _b_radious6 _padd_b24 _padd_t24 _mar_b16">
      <div className="_feed_inner_timeline_content _padd_r24 _padd_l24">
        ...
        <div className="_feed_inner_timeline_post_top">
          <div className="_feed_inner_timeline_post_box">
            <div className="_feed_inner_timeline_post_box_image">
              <img
                src={getImageUrl(feed.author?.image) || "assets/images/post_img.png"}
                alt=""
                className="_post_img"
              />
            </div>
            <div className="_feed_inner_timeline_post_box_txt">
              ...
              <p className="_feed_inner_timeline_post_box_para">
                ...
                <a href="#0">{feed.visibility === "PUBLIC" ? "Public" : "Private"}</a>
              </p>
            </div>
          </div>
          <div className="_feed_inner_timeline_post_box_dropdown">
            <div className="_feed_timeline_post_dropdown">
              ...
              <ul className="dropdown-menu _feed_timeline_dropdown">
                <li className="_feed_timeline_dropdown_item">
                  <button className="_feed_timeline_dropdown_link dropdown-item" type="button">
                    <span>
                ...
                {isOwner && (
                  <li className="_feed_timeline_dropdown_item">
                    <button
                      className="_feed_timeline_dropdown_link dropdown-item"
                      type="button"
        ...
        {/* Post Title / Content */}
        {feed.content && (
          <h4 className="_feed_inner_timeline_post_title">{feed.content}</h4>
        )}
        {/* Post Image */}
        {feed.image && (
      ...
      <div className="_feed_inner_timeline_reaction">
        <button
          ...
          onClick={() => !liking && toggleLike(feed.id)}
          disabled={liking}
        >
      ...
      {showComments && (
        <div className="_feed_inner_timeline_cooment_area">
          <div className="_feed_inner_comment_box">
            <form className="_feed_inner_comment_box_form" onSubmit={commentForm.handleSubmit(onSubmitComment)}>
              <div className="_feed_inner_comment_box_content">
                <div className="_feed_inner_comment_box_content_image">
                  <img
                    src={getImageUrl(user?.image) || "assets/images/comment_img.png"}
                    alt=""
                ...
                <div className="_feed_inner_comment_box_content_txt">
                  ...
                  {commentForm.formState.errors.content && (
                    <small className="text-danger">{commentForm.formState.errors.content.message}</small>
                  )}
                </div>
              </div>
              ...
              <input type="hidden" {...commentForm.register("feedId")} />
            </form>
          </div>
          {/* Comments List */}
          {comments.length > 0 && (
...
function CommentItem({
  ...
  const replyForm = useForm<TCreateComment>({
    resolver: zodResolver(ZCreateComment),
    defaultValues: { feedId, content: "", parentId: comment.id },
  });

  const onSubmitReply = (data: TCreateComment) => {
    createComment(data, {
      onSuccess: () => {
        replyForm.reset({ feedId, content: "", parentId: comment.id });
        setShowReply(false);
      },
    });
  };

  const liked = comment.likes?.some((l: any) => l.userId === currentUserId);
    ...
    <div className="_comment_main">
      <div className="_comment_image">
        <a href="profile.html" className="_comment_image_link">
          <img
            src={getImageUrl(comment.author?.image) || "assets/images/txt_img.png"}
            alt=""
      ...
      <div className="_comment_area">
        ...
        {showReply && (
          <div className="_feed_inner_comment_box">
            <form className="_feed_inner_comment_box_form" onSubmit={replyForm.handleSubmit(onSubmitReply)}>
              <div className="_feed_inner_comment_box_content">
                <div className="_feed_inner_comment_box_content_image">
                  <img
                    src={getImageUrl(user?.image) || "assets/images/comment_img.png"}
              ...
              <input type="hidden" {...replyForm.register("feedId")} />
            </form>
          </div>
        )}
        {/* Nested Replies */}
        {comment.replies?.map((reply: any) => (
          <div key={reply.id} className="_comment_main" style={{ marginLeft: "2rem", marginTop: "0.5rem" }}>
            <div className="_comment_image">
              <a href="profile.html" className="_comment_image_link">
                <img
                  src={getImageUrl(reply.author?.image) || "assets/images/txt_img.png"}
                  alt=""
                  className="_comment_img1"
                />
              </a>
...
export default function Feed() {
  ...
  const pagination = feedsData?.pagination || {};
  ...
  const {
    ...
    resolver: zodResolver(ZCreateFeed),
    defaultValues: { content: "", visibility: "PUBLIC" },
  });

  const { mutate: createFeed, isPending: isCreating } = useCreateFeed();

  const onSubmitFeed = (formData: TCreateFeed) => {
    createFeed(
      { ...formData, image: selectedImage || undefined },
      {
        onSuccess: () => {
